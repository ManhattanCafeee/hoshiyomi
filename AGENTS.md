...
```bash
pnpm install               # node-linker=hoisted + allowBuilds live in pnpm-workspace.yaml (pnpm 12 project settings)
...
| `uniapp/pnpm-workspace.yaml` | `nodeLinker: hoisted` + overrides + allowBuilds — pnpm 12 reads project settings only from here (`uniapp/.npmrc` removed) |
...
- **Node/pnpm**: Node ≥ 26 with pnpm 12 — both workspace roots pin `"packageManager": "pnpm@12.4.1"` and `"engines": { "node": ">=26", "pnpm": ">=12" }`, and CI installs that pair through `pnpm/setup@v3` (`runtime: node@26`, pnpm store + lockfile-verification caches). pnpm 12 treats `engines` as **advisory** (measured: `engines.pnpm: ">=99"` and a `node --version` of 22 both install and run scripts fine), while pnpm 11 still hard-fails on `engines.pnpm` with `ERR_PNPM_UNSUPPORTED_ENGINE` — the enforcing pins are `packageManager` and the CI runtime. Both lockfiles are two-document (the leading env document holds the resolved pnpm pin), so bumping `packageManager` means regenerating and committing them or the frozen CI install fails with `ERR_PNPM_FROZEN_LOCKFILE_WITH_OUTDATED_LOCKFILE`. `frontend/` is one workspace (lockfile v9); `uniapp/` is an **independent** project: own lockfile, own `pnpm-workspace.yaml` carrying `nodeLinker: hoisted` (uni-app's build chain forces `preserveSymlinks: true`, so transitive deps must be flat at root — do NOT remove this), never mix pnpm invocations between the two dirs. No corepack.
...
- **CI**: `.github/workflows/ci.yml` — three jobs: `backend` (`cargo audit` via `.cargo/audit.toml` / fmt / clippy `-D warnings` / test / `docs/openapi.json` must equal `dump_openapi` output), `admin` (`pnpm audit` at `high` → `pnpm check` → SDK drift check → type-check → build) and `uniapp` (`pnpm audit --prod` at `critical` → `pnpm lint` → SDK drift check → type-check → H5 build → `build wx` for the MP-WEIXIN branches). Both frontend jobs install pnpm 12 + Node 26 through `pnpm/setup@v3`; `require-lockfile: true` keeps the install frozen, so both lockfiles must be committed up to date. uniapp audits prod-only because its frozen uni-app/Vite 5 toolchain carries unfixable dev-only advisories; audit thresholds live in each `pnpm-workspace.yaml` under `audit.level`. Run the same commands locally — they need no database (only `cargo audit` needs network for the advisory DB).
...
- **Admin frontend**: no unit tests — `pnpm check` (lint + prettier) + `pnpm type-check` + `pnpm --filter admin build` + `pnpm audit` (threshold `high`, from `audit.level` in `frontend/pnpm-workspace.yaml`) are the gates, and CI runs all of them; behavior verified manually in the browser (login → dashboard → users CRUD → profile → logout).
- **Uniapp**: no unit tests — `pnpm lint` + `pnpm type-check` + `pnpm build` (H5) + `pnpm build wx` (MP-WEIXIN conditional-compilation branches) + `pnpm audit --prod` (threshold `critical`) are the gates, and CI runs all of them; behavior verified manually (register → login → workbench → profile → logout).
- **Coverage**: no coverage config. CI (`ci.yml`) enforces fmt/clippy/test + `cargo audit` + RustSpec/SDK drift on every push, plus the admin/uniapp lint·type-check·build·audit gates; no coverage gate.
