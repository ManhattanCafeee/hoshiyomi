...
import { CalendarRoot, injectConfigProviderContext, useDateFormatter, useForwardPropsEmits } from 'reka-ui'
...
const configContext = injectConfigProviderContext({ locale: ref('en') })
const formatter = useDateFormatter(props.locale ?? configContext.locale?.value ?? 'en')
...
<template>
        ...
        <NativeSelect
          ...
          @change="
            (e: Event) => {
              ...
              if (noGrid) emits('update:modelValue', placeholder)
            }
          "
        >
          <NativeSelectOption
  ...
  >
    ...
    <div v-if="!noGrid" class="flex flex-col gap-y-4 mt-4 sm:flex-row sm:gap-x-4 sm:gap-y-0">
